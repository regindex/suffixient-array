...
class CTriePP {
   ...
   public:
        ...
        while (true) {
            ...
            if (lcpLength == targetNode.subText_.size())
            {
                ...
                if (lcpLength == newSubText.size())
                {
                    ...
                    else { WARN("Already exist insert key "); exit(1); }
                    break;
                }
                newSubText =
    ...
    inline std::tuple<Ulong,Int,bool> locateLongestPrefix(const std::string &pattern) const
    {
    ...
    inline std::tuple<NodeIndex,Int,bool> 
                getKeyLongestPrefix(const Node &node, const Ulong &key) const
    {
        return node.children_ == nullptr
                   ? std::make_tuple(Ulong(-1),0,false)
