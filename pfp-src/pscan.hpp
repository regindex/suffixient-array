...
static void *mt_parse(void *dx)
{
  ...
  if((d->size - d->end)==0) { word.append(1,Dollar); }// no need to reach the next kr-window
  else
  { 
    // reach the next breaking window  
    std::streampos pos = d->end;
    while( pos > 0 )
    {
      std::streamsize to_read = std::min(CHUNK_SIZE, static_cast<std::streamsize>(pos));
      pos -= to_read;
      f.seekg(pos);
      ...
      if(i>=0) break;
      else{ if(pos==0){ f.close(); return NULL; } } // reached EOF without finding a breaking point nothing to do
    }
  ...
  uint64_t pos = d->size - d->end;                      // ending position+1 in text of previous word
  if((d->size - d->end)>0) pos += d->skipped + arg->w;  // or 0 for the first word  
  assert(IBYTES<=sizeof(pos));                          // IBYTES bytes of pos are written to the sa info file 

  std::streampos pos_ = d->end - d->parsed - d->skipped;
  while( pos_ > 0 )
  {
    std::streamsize to_read = std::min(CHUNK_SIZE, static_cast<std::streamsize>(pos_));
    pos_ -= to_read;
    f.seekg(pos_);
...
static uint64_t mt_process_file(Args& arg, MTmaps &mtmaps)
{
  ...
  for(int i=0;i<arg.th;i++) {
    ...
    td[i].size = size; // file size
    assert(td[i].end<=size);
    // open the 1st pass parsing file 
    td[i].tmp_parse_file = open_aux_file_num(arg.inputFileName.c_str(),EXTPARS0,i_,"wb");
    // open output file containing the char at position -(w+1) of each word
    td[i].last_file = open_aux_file_num(arg.inputFileName.c_str(),EXTLST,i_,"wb");  
    // if requested open file containing the ending position+1 of each word
    td[i].sa_file = arg.SAinfo ?open_aux_file_num(arg.inputFileName.c_str(),EXTSAI,i_,"wb") : NULL;
